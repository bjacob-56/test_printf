...
/*   Updated: 2020/11/27 17:07:56 by bjacob           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
//void	ft_dprintf(int fd, const char *format, char *str)
{
	va_list	ap;
//	char	*str;
...
{
//	ft_dprintf(1, "\033[0;32m");
//	ft_dprintf(1, "return : OK\n\n");
//	ft_dprintf(1, "\033[0m");
//	printf("\n");
	*i += 1;
}
...
void	print_undef ()
{
//	ft_dprintf(1, "\033[3;32m");
//	ft_dprintf(1, "printf    :\tundefined\n\n");
	ft_dprintf(1, "printf    :\tundefined\n");
//	ft_dprintf(1, "\033[0m");
}

void	print_testing (int fd, char *str)
{
//	(void)str;
//	ft_dprintf(1, "\033[3;90m");
	dprintf(fd, "testing \t%s\n", str);
//	ft_dprintf(fd, "%s\n", str);
//	ft_dprintf(1, "\033[0m");
}
...
void	print_result(size_t tested, size_t ok, size_t ko)
{
	...
	ft_dprintf(1, "\033[1;90m");
	ft_dprintf(1, "\tTESTED : %zu\n", tested);
	ft_dprintf(1, "\033[0m");
	if (pourcentok == 100)
	{
		ft_dprintf(1, "\033[1;32m");
		ft_dprintf(1, "\t%zu TEST FULL PASSED\n", ok);
		ft_dprintf(1, "\033[0m");
	}
	else 
	{
		ft_dprintf(1, "\033[1;31m");
		ft_dprintf(1, "\t%zu%% failed\n", 100 -pourcentok);
		ft_dprintf(1, "\t%zu errors found\n", ko);
		ft_dprintf(1, "\033[0m");
	}
...
int		main(int ac, char **av)
{
	...
	fd_test = open("tests1", O_CREAT, S_IRWXU);
	if (fd_test > 0)
		close(fd_test);
	fd_test = open("tests1", O_RDWR);
...
//	fd_test = 1;
	...
	{
//	{ft_dprintf(1, ">--------------- CLASSIC TEST ---------------<\n\n");
	
	tested++;
	print_testing(fd_test, "bonjour");
...
//ft_dprintf(1, ">------------------ U TEST ------------------<\n\n");
	...
	tested++;
	print_testing(fd_test, "\"|%-50s|\\n\", \"bonjour");
	if (printf("printf    :\t|%-50s|\n", "bonjour") == ft_printf("ft_printf :\t|%-50s|\n", "bonjour"))
		print_ok(&testok);
	else
		print_error(&testko);

	print_testing(fd_test, "\"|%-050s|\\n\", \"bonjour");
	...
	tested++;
	print_testing(fd_test, "\"|%46.0s|\\n\", \"\"");
	if (printf("printf    :\t|%46.0s|\n", "") == ft_printf("ft_printf :\t|%46.0s|\n", ""))
		print_ok(&testok);
	else
		print_error(&testko);

	print_testing(fd_test, "\"|%0.46s|\\n\", \"\"");
	...
	//ft_dprintf(1, ">-------------- Pourcent TEST ---------------<\n\n");

	tested++;
	print_testing(fd_test, "\"|%54%|\\n\"");
	if (printf("printf    :\t|%54%|\n") == ft_printf("ft_printf :\t|%54%|\n"))
		print_ok(&testok);
	else
		print_error(&testko);

	//ft_dprintf(1, ">---------------- MORGAN TEST ---------------<\n\n");

	tested++;
	print_testing(fd_test, "\"|%.3s|\\n\", NULL");
	...
	//ft_dprintf(1, ">------------------ MIX TEST ----------------<\n\n");

	tested++;
	print_testing(fd_test, "\"|%s %d %p %x %X %i %u %c|\n\", \"bonjour\", 50, \"bonjour\", 168, -168, 54, 87, \'c\'");
	...
	{
		tested++;
		print_testing(fd_test, "\"%s\",  random");
		if (printf("printf    :\t|%s|\n", str[i]) == ft_printf("ft_printf :\t|%s|\n", str[i]))
			print_ok(&testok);
		else
			print_error(&testko);
	...
	{
		tested++;
		print_testing(fd_test, "\"|%010.0d|\\n\", random");
		if (printf("printf    :\t|%010.0d|\n", nb[i]) == ft_printf("ft_printf :\t|%010.0d|\n", nb[i]))
			print_ok(&testok);
		else
			print_error(&testko);
		i++;
	...
	if(ft_c_in_str('i', av[1])|| ft_c_in_str('a', av[1]))
	{
		...
		{
			tested++;
			print_testing(fd_test, "\"%3i\", random");
			if ((printf("printf    :\t|%-3i|\n", nb[i])) == (ft_printf("ft_printf :\t|%-3i|\n", nb[i])))
				print_ok(&testok);
			else
				print_error(&testko);
			i++;
		...
		{
			tested++;
			print_testing(fd_test, "\"%01i\", random");
			if ((printf("printf    :\t|%01i|\n", nb[i])) == (ft_printf("ft_printf :\t|%01i|\n", nb[i])))
				print_ok(&testok);
			else
				print_error(&testko);
			i++;
		...
		{
			tested++;
			print_testing(fd_test, "\"%10.1i\", random");
			if ((printf("printf    :\t|%2.1i|\n", nb[i])) == (ft_printf("ft_printf :\t|%2.1i|\n", nb[i])))
				print_ok(&testok);
			else
				print_error(&testko);
			i++;
		...
		{
			tested++;
			print_testing(fd_test, "\"%1.10i\", random");
			if ((printf("printf    :\t|%1.10i|\n", nb[i])) == (ft_printf("ft_printf :\t|%1.10i|\n", nb[i])))
				print_ok(&testok);
			else
				print_error(&testko);
			i++;
...
if(ft_c_in_str('c', av[1])|| ft_c_in_str('a', av[1]))
	{//ft_dprintf(1, ">---------------- ASCII TEST ---------------<\n\n");

	print_testing(fd_test, "all ascii char");
	unsigned int u;
